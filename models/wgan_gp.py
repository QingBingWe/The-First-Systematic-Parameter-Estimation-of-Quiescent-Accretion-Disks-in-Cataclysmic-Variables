...
def train(dataset, epochs, generator, critic, save_interval=100, critic_iterations=5, lambda_gp=10):
    ...
    # 创建保存生成样本的目录
    if not os.path.exists('../checkpoints/check2/wgangp'):
        os.makedirs('../checkpoints/check2/wgangp')
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % save_interval == 0:
            ...
            generator.save_weights(f'../checkpoints/check2/wgangp/generator_{epoch+1}.h5')
    
    # 绘制损失曲线
    with open('../checkpoints/check2/wgangp/loss.txt', "w") as f:
...
def generate_and_save_spectra(generator, epoch, latent_dim, seq_length):
    ...
    plt.tight_layout()
    plt.savefig(f'../checkpoints/check2/wgangp/spectrum_epoch_{epoch}.png')
    plt.close()
...
def plot_losses(gen_losses, critic_losses):
    ...
    plt.legend()
    plt.grid(alpha=0.3)
    plt.savefig('../checkpoints/check2/wgangp/wgan_gp_loss_curve.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    if Train:
        ...
        # 保存最终模型
        trained_generator.save_weights('../checkpoints/check2/wgangp/generator.h5')
        trained_critic.save_weights('../checkpoints/check2/wgangp/critic.h5')
        print("模型权重已保存")
    else:
        cloudy_data = np.load("../data/cloudy_data/cloudy_dataset_256.npy")
        model_path ="../checkpoints/check2/wgangp/generator.h5"
        ...
        np.save("../data/generated_data_1/wganGP_5000_cloudy_dataset_256.npy", new_cloudy_data)
